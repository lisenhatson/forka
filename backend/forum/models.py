...
# Add this class to backend/forum/models.py
# (it reuses the same random/string/timezone/timedelta imports already at the top of that file)


class LoginMFACode(models.Model):
    """
    One-time code emailed at login time, after the password check passes.
    Mirrors EmailVerification / PasswordReset in structure and behavior.
    """
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name='login_mfa_codes',
    )
    code = models.CharField(max_length=6)
    created_at = models.DateTimeField(auto_now_add=True)
    expires_at = models.DateTimeField()
    ...
    def __str__(self):
        return f"Login MFA code for {self.user.username}"
